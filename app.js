...
const initAppSet = async (args) => {
  ...
  //Start breath
  console.log(`SET server/environment is ${conf.server_code}/${env.environment}. Get New infomation!!!`);
  ...
  app.use(function(err, req, res) {
    ...
    if (err.status === 404) {
      ...
      result = {
        type: "API",
        status_code: err?.status_code || code.ERR_A_SYSTEM_990,
        status_msg : err?.status_msg || status.ERR_A_SYSTEM_990,
        approval: false,
        http_status : err.status,
        message: err.message,
        stack : err.stack
